...
<div class="container">
    <div class="row">
        <div class="col-6 p-5 border border-dark ">
            ...
            <form class="needs-validation" id="billing" method="POST" action="<?php echo $this->getUrl()->getUrl('save') ?>">
                <table border="1">
                    <tr>
                ...
                <button class="btn btn-primary" type="button" name="submit" onclick="mage.setForm('#billing').load()">Save</button>
            </form>
        </div>
        <div class="col-6 p-5 border border-dark ">
            <h4 class="mb-3">Shipping address</h4>
            <form class="needs-validation" id="shipping" method="POST" action="<?php echo $this->getUrl()->getUrl('save') ?>">
                <table border="1">
                    <tr>
                        <label><strong>Address *</strong></label>
                ...
                <button class="btn btn-primary" type="button" name="submit" onclick="mage.setForm('#shipping').load()">Save</button>
            </form>
        </div>
    </div>
        <form method="POST" id="details" action="<?php echo $this->getUrl()->getUrl('saveDetails') ?>">
            <div class="col-md-4 order-md-2 mb-4">
                <h4 class="d-flex justify-content-between align-items-center mb-3">
                    <span class="text-muted">Products Detail</span>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <div style="display: none;">
                                    <input type="text" name="product[total]"
                                        value="<?php echo $this->getPrice() - $this->getTotalPrice() ?>">
                                </div>
                                <div style="display: none;">
                                    <input type="text" name="product[discount]"
                                        value="<?php echo $this->getTotalPrice()  ?>">
                                </div>
                                <td></td>
                ...
                <ul class="list-group mb-3">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th scope="row"><input type="radio" id="shipping" name="shipping[shippingId]"
                                        value="<?php echo $shippingMethod->shippingId ?>"
                                        <?php if($shippingMethod->shippingId == $this->getCart()->getOriginalData()['shippingmethodId']) { ?>
                                        checked <?php } ?>></th>
                                <td><?php echo $shippingMethod->name ?></td>
                                <td><input id="shippingData" type="number"
                                        name="shipping[amount][<?php echo $shippingMethod->shippingId ?>]"
                                        value="<?php echo $shippingMethod->amount ?>"></td>
                ...
                <ul class="list-group mb-3">
                    ...
                    <li class="list-group-item d-flex justify-content-between lh-condensed">
                        <div>
                            <input type="radio" name="payment" value="<?php echo $paymentMethod->paymentId ?>"
                                <?php if($paymentMethod->paymentId == $this->getCart()->getOriginalData()['paymentmethodId']) { ?>
                                checked <?php } ?>>
            ...
            <button class="btn btn-primary" type="button" name="submit" onclick="mage.setForm('#details').load()">Save</button>
            </div>
        </form>


        <hr class="mb-4">
        <form method="POST" id="order" action="<?php echo $this->getUrl()->getUrl('save', 'Order') ?>">
            <button class="btn btn-primary btn-lg btn-block" type="button" onclick="mage.setForm('#order').load()">Place Order</button>
        </form>
    </div>
</div>
</div>
